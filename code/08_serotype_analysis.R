...
#
# Input:
#   results/fit_model3.rds (or fit_model2.rds as fallback), data/model_data.rds
#   Serotype data:
...
# Load Model 3 (climate-only) if available, fall back to Model 2
if (file.exists("../results/fit_model3.rds")) {
  ...
  model_label <- "Model 2 (full)"
} else {
  stop("No model fit found. Run 13_fit_model3.R or 06_fit_models.R first.")
}
...
# Negative lag = f_residual leads (Rt rises before indicator changes).
# ==============================================================================
...
# Print key lags
# Convention: positive lag = x leads y (entropy leads Rt)
# In R's ccf(x, y), lag h means x(t+h) is correlated with y(t),
# i.e., positive h means y leads x. Our x=f_residual, y=entropy.
# So positive lag = f_residual leads; negative lag = entropy leads.
cat("\nCross-correlation: f_residual vs Shannon entropy:\n")
cat("  (Positive lag = Rt leads entropy; Negative lag = entropy leads Rt)\n")
for (lag_val in c(-6, -3, -1, 0, 1, 3, 6)) {
...
p_ccf <- ggplot(ccf_plot_df, aes(x = lag, y = ccf)) +
  ...
  labs(
    ...
    subtitle = paste(
      "Positive lag = Rt leads; Negative lag = serotype indicator leads Rt.",
      "Dashed lines: 95% bounds."
    ),
    x = "Lag (months)",
    y = "Cross-correlation"
  )
