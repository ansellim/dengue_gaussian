...
p_logratio <- ggplot(transition, aes(x = year_month)) +
  # COVID phase shading (gradient of restriction strength preserved in fill)
  annotate("rect",
           xmin = covid_phases$start, xmax = covid_phases$end,
           ymin = -Inf, ymax = Inf,
           fill = covid_phases$fill, alpha = 0.3) +
  # Single horizontal label spanning the combined Apr--Dec 2020 window
  annotate("text",
           x = as.Date("2020-08-15"),
           y = max(transition$log_ratio_raw, na.rm = TRUE) * 0.95,
           label = "COVID restrictions",
           size = 3.2, fontface = "bold", color = "gray15", hjust = 0.5) +
  # Parity line
...
# ==============================================================================
# 3b. DENV-2 AND DENV-3 MONTHLY PROPORTIONS (visual anchor for replacement)
# ==============================================================================
#
# Shows what fraction of typed cases each serotype accounts for month by month,
# so the reader can see the crossover from DENV-2 dominance to DENV-3 dominance
# in the same COVID-phase shaded window used by the log-ratio and Rt panels.
# D2 + D3 do not quite sum to 1 because DENV-1 and DENV-4 are present in small
# amounts; the y-axis is the raw share of typed cases.
# ==============================================================================

prop_long <- transition |>
  select(year_month, D2_prop, D3_prop, D2_smooth, D3_smooth) |>
  pivot_longer(
    cols = c(D2_prop, D3_prop, D2_smooth, D3_smooth),
    names_to  = c("serotype", ".value"),
    names_pattern = "(D[23])_(prop|smooth)"
  ) |>
  mutate(serotype = recode(serotype, "D2" = "DENV-2", "D3" = "DENV-3"))

sero_colors <- c("DENV-2" = "#D95F02", "DENV-3" = "#7570B3")

p_proportions <- ggplot(prop_long, aes(x = year_month, color = serotype)) +
  # COVID phase shading (gradient of restriction strength preserved in fill)
  annotate("rect",
           xmin = covid_phases$start, xmax = covid_phases$end,
           ymin = -Inf, ymax = Inf,
           fill = covid_phases$fill, alpha = 0.25) +
  # Single horizontal label spanning the combined Apr--Dec 2020 window
  annotate("text",
           x = as.Date("2020-08-15"),
           y = 0.93,
  ...
  # Raw monthly proportions as points
  geom_point(aes(y = prop), size = 2, alpha = 0.55) +
  # GAM-smoothed trajectories as lines
  geom_line(aes(y = smooth), linewidth = 1.2) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     limits = c(0, 1), expand = c(0, 0)) +
  scale_color_manual(values = sero_colors, name = NULL) +
  labs(
    title = "DENV-2 and DENV-3 monthly proportions",
    subtitle = "Points: raw monthly shares of typed cases. Lines: GAM-smoothed. Oct 2019 -- Sep 2021.",
    x = "Date",
    y = "Share of typed cases",
    caption = "DENV-1 and DENV-4 are present at low levels in this window; D2 + D3 need not sum to 100%."
  ) +
  theme(legend.position = "top",
        plot.caption = element_text(size = 8, hjust = 0))

ggsave("../results/figures/serotype_proportions_transition.png", p_proportions,
       width = 10, height = 5, dpi = 150)
cat("  Saved: results/figures/serotype_proportions_transition.png\n")
...
# A = proportions (the visual anchor), B = piecewise selection (adds the
# fitted slopes and CIs over the log-ratio), C = Rt decomposition.
# The raw log-ratio panel is omitted here because panel B already shows the
# log-ratio with the regression lines overlaid --- it would be redundant.
pA <- p_proportions + labs(title = "A) DENV-2 and DENV-3 monthly proportions")
pB <- p_piecewise   + labs(title = "B) Piecewise selection coefficients (log-ratio)")
